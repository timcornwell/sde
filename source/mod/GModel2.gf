 ...
 *
 * The model consists of N Gaussians, each with 2 free parameters and 4
 * fixed parameters.  One of the free parameters, is "radius", which is
 * enforced by allowing both BMaj and BMin to vary, but requiring that
 * their ratio be help fixed.
 *
 * Flux   (Jy)		(free)
 * BMaj   (asec)	(free)
 * BMin   (asec)	(free)
...
main() {
  ...
  for (i=1; i<=Ncomp; i=i+1) {
    ...
    diff = Ratio[i] - BMaj[i] / BMin[i];
    exportconstraint(diff);
  }
