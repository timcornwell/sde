...
C++
C @(#)imgclean.f	1.5    6/10/93
C
      SUBROUTINE IMGCLEAN (DRT, PSF, CLN, RES, BOX)
C
CD Clean an image.
C
C
...
C	Tweaked up support for 1D images
...
C
      IF (NREAL.EQ.1) THEN
         CALL PIX1DRCL (MEMR(DADD), MEMR(PADD), MEMR(BADD), DNAXIS(1),
     1      PNAXIS(1), GAIN, NSUB, BSUB, 
     2      FLUX, MEMR(CADD), MEMR(RADD), ANSUB, MAXRES, TFLUX,
     $      MEMR(CLADD), MEMI(CXLADD))
      ELSE IF (NREAL.EQ.2) THEN
         CALL PIX2DRCL (MEMR(DADD), MEMR(PADD), MEMR(BADD), DNAXIS(1),
     1      DNAXIS(2), PNAXIS(1), PNAXIS(2), GAIN, NSUB, BSUB,
     2      FLUX, MEMR(CADD), MEMR(RADD), ANSUB, MAXRES, TFLUX,
     $      MEMR(CLADD), MEMI(CXLADD), MEMI(CYLADD))
      ELSE IF (NREAL.EQ.3) THEN
         CALL PIX3DRCL (MEMR(DADD), MEMR(PADD), MEMR(BADD), DNAXIS(1),
     1      DNAXIS(2), DNAXIS(3), PNAXIS(1), PNAXIS(2), PNAXIS(3),
     2      GAIN, NSUB, BSUB, FLUX, MEMR(CADD), MEMR(RADD), ANSUB, 
     3      MAXRES, TFLUX, MEMR(CLADD), MEMI(CXLADD), MEMI(CYLADD),
     $      MEMI(CZLADD))
      ELSE
