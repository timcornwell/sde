...
C++
C @(#)imgnncl1.f	1.1    6/7/93
C
      SUBROUTINE IMGNNCL1 (DRT, PSF, CLN, RES, BOX)
C
CD Clean an image.  Non-negative CCs  (old style backtracking)
C
...
C
      ...
      INTEGER		CLADD, CXLADD, CYLADD, CZLADD, WADD
C
...
C
     ...
     1     CXLADD)
      ...
      END IF
C
      CALL ARRCOPY (RES, 'NNCLN-TMP')
      WADD = DATADD ('NNCLN-TMP')
C
C Finally do something
C
      IF (NREAL.EQ.2) THEN
         CALL PIX2DRN1 (MEMR(DADD), MEMR(PADD), MEMR(BADD),
     $        DNAXIS(1), DNAXIS(2), PNAXIS(1), PNAXIS(2), GAIN, NSUB,
     $        BSUB, FLUX, MEMR(CADD), MEMR(RADD), MEMR(WADD), ANSUB,
