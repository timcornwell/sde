...
C++
C @(#)img3ft3.f	1.2 9/18/92
C
      SUBROUTINE IMG3FT3 (VIS, NVIS, WT, U, V, W, IMAGE, NX, 
     1   NY, NZ, REFX, DELTX, REFY, DELTY, REFZ, DELTZ, SHIFT)
C
...
C
      CHARACTER*(*)	ROUTINE
      PARAMETER		(ROUTINE = 'IMG3FT3')
C
      INTEGER		IX, IY, IZ, IVIS, NCS, COFFSET, CSADD
...
C
      ...
      DO 20 IY = 1, NY
         DO 10 IX = 1, NX
            IF(IMAGE(IX,IY,IZ).NE.0.0) THEN
               X = D2R * DELTX * (FLOAT(IX) - REFX)
               Y = D2R * DELTY * (FLOAT(IY) - REFY)
C
C Z should be Z = SQRT(1-X**2-Y**2)-1.0
C 
               Z = D2R * DELTZ * (FLOAT(IZ) - REFZ)
               DO 100 IVIS = 1, NVIS
