...
C
     ...
     $         STRM2(VIS, 'MOD/'//STOKES(1:1)//'/VIS'))
            CALL MOSDFTPB (VIS, 'MOD/'//STOKES(1:1), 'TmpVM', DBCHISQ)
...
C++
C
      SUBROUTINE MOSDFTPB (VIS, CLASS, IMAGE, CHISQ)
C
C Direct Fourier transform to Image from Vis. data after applying
...
C	DBCHISQ		REAL	output	Chisq before 2nd PB
C Audit trail:
C	Original version: Audit trail comments go on this line
C	and successive lines
C				T.J.Cornwell	Jan 5 1989
...
C
     ...
     1			IROTA(SYSMXDIM)
      DOUBLE PRECISION	IRVAL(SYSMXDIM), FREQ, DTMP
      INTEGER		VNAX, VNAXIS(SYSMXDIM)
      LOGICAL		DATEXIST, NEW
      INTEGER		NPB
      INTEGER 		NDUMMY, RNAX, CRDRNAX, I, STRSEARC
...
C
      IF (DATEXIST (STRM2(VIS, 'TELESCOP'))) THEN
         CALL DATGETC (VIS, 'TELESCOP', TELESCOP, 1, NDUMMY)
      ELSE
...
C
         ...
         ELSE IF (TELESCOP(1:4).EQ.'VLBA') THEN
            ...
            RCONST = RTOA * (3E8/(FREQ*1E9))/(PI * 25.0)
            BMX = (3600.0*IDELT(1)/RCONST)**2
            BMY = (3600.0*IDELT(2)/RCONST)**2
         ELSE IF (TELESCOP(1:4).EQ.'DISH') THEN
            CALL ARRAIRYB (NPB, RADMAX, PBADD, NEW)
            RCONST = RTOA * (3E8/(FREQ*1E9))/(PI * 15.0)
            BMX = (3600.0*IDELT(1)/RCONST)**2
            BMY = (3600.0*IDELT(2)/RCONST)**2
         ELSE IF (TELESCOP(1:4).EQ.'AIRY') THEN
            ...
            BMX = (3600.0*IDELT(1)/RCONST)**2
            BMY = (3600.0*IDELT(2)/RCONST)**2
         ELSE IF (TELESCOP(1:4).EQ.'AIRYB') THEN
            CALL ARRAIRYB (NPB, RADMAX, PBADD, NEW)
            CALL DATGETR(VIS, 'TELDIAM', TELDIAM, 1, NDUMMY)
            RCONST = RTOA * (3E8/(FREQ*1E9))/(PI * TELDIAM)
            BMX = (3600.0*IDELT(1)/RCONST)**2
         ...
         ELSE IF (TELESCOP(1:3).EQ.'12M') THEN
            CALL ARRAIRYB (NPB, RADMAX, PBADD, NEW)
            RCONST = RTOA * (3E8/(FREQ*1E9))/(PI * 12.0)
            BMX = (3600.0*IDELT(1)/RCONST)**2
            BMY = (3600.0*IDELT(2)/RCONST)**2
         ELSE
            CALL ERRREPOR (ERRBDARG, ROUTINE,
...
C++
C
      SUBROUTINE MOSDFTP2 (VIS, NVIS, WT, U, V, IMAGE, NX, NY, 
     1   REFX, DELTX, REFY, DELTY, CX, CY, BMX, BMY, NPB, PBARR,
     2   RADMAX, CHISQ, SUMWT)
C
C Pixel-level DFT routine VIS->IMG in 2D, applies primary beam after
C transformation
C
...
C
      ...
      REAL		PBARR(*), RADMAX, CHISQ, SUMWT
      COMPLEX		VIS(*)
C
      CHARACTER*(*)	ROUTINE
      PARAMETER		(ROUTINE = 'MOSDFTP2')
C
