...
C++
C @(#)crdhalf.f	1.3    11/7/90
C
      SUBROUTINE CRDHALF (NAME, BLC, TRC)
C
CD Adjust BLC, TRC to be no more than half an axis.
C
C
...
C
         ...
         IF(NAXIS(IAX).GT.1) THEN
            ...
            IF(TRC(IAX).EQ.1) THEN
               TRC(IAX) = 3 * NAXIS(IAX) / 4
            END IF
            IDEL = MIN(TRC(IAX)-BLC(IAX)+1,NAXIS(IAX)/2)
