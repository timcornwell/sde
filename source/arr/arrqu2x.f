...
C++
C @(#)arrqu2x.f	1.1	 2/14/91
C
      SUBROUTINE ARRQU2X (REAL1, REAL2, COMP)
C
CD Makes a complex array out of two real arrays: COMP = REAL1 + iREAL2
C
C Arguments: CALL ARRQU2X (REAL1, REAL2, COMP)
C
C	REAL1	CH*(*)	input	Name of REAL part of image
C	REAL2	CH*(*)	input	Name of Imaginary part of image
...
C
      ...
      ENDIF
C         
      CALL PIXQU2X (NT, MEMX(XADD), MEMR(R1ADD), MEMR(R2ADD) )
C
      IF (ERROR) GOTO 999
