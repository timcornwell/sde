...
C++
C @(#)arrsel.f	1.1    7/13/94
C
       SUBROUTINE ARRSEL (A1, A2, A3)
C
CD Select A3 =   A1,  if A2 >  0
C         A3 = -|A1|, if A2 <= 0
C
...
C
      IF (T1.EQ.'R') THEN
         DO 100 I = 0, NT-1
            IF (MEMR(ADD2+I).GT.0.0) THEN
               MEMR(ADD3+I) = MEMR(ADD1+I)
            ELSE
               MEMR(ADD3+I) = -ABS(MEMR(ADD1+I))
            END IF
 100     CONTINUE
      ELSE
