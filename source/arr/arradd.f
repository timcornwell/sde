...
C++
C @(#)arradd.f	1.4    7/23/92
C
      SUBROUTINE ARRADD (A1, A2, A3)
C
...
C
      IF (T1.EQ.'R') THEN
         CALL PIXRADD (MEMR(ADD1), MEMR(ADD2), MEMR(ADD3), NT)
      ELSE IF (T1.EQ.'X') THEN
         CALL PIXXADD (MEMX(ADD1), MEMX(ADD2), MEMX(ADD3), NT)
      ELSE IF (T1.EQ.'I') THEN
         CALL PIXIADD (MEMI(ADD1), MEMI(ADD2), MEMI(ADD3), NT)
      ELSE IF (T1.EQ.'D') THEN
         CALL PIXDADD (MEMD(ADD1), MEMD(ADD2), MEMD(ADD3), NT)
      ELSE
         CALL ERRREPOR (ERRBDARG, ROUTINE,
