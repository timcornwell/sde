...
C++
C @(#)arrpc2x.f	1.1	 2/14/91
C
      SUBROUTINE ARRPC2X (REAL1, REAL2, COMP)
C
CD Makes a complex array out of two real arrays: COMP = REAL1 * EXP(i 2 REAL2)
C
C Arguments: CALL ARRPC2X (REAL1, REAL2, COMP)
C
C	REAL1	CH*(*)	input	Name of AMP
C	REAL2	CH*(*)	input	Name of CHI
...
C
      ...
      ENDIF
C         
      CALL PIXPC2X (NT, MEMX(XADD), MEMR(R1ADD), MEMR(R2ADD) )
C
      IF (ERROR) GOTO 999
