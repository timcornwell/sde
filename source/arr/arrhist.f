...
C++
C @(#)arrhist.f	1.3    11/7/90
C
      SUBROUTINE ARRHIST (A, HIST)
C
CD Make a histogram of an array
C
C
...
C
      IF (TA.EQ.'R') THEN
         CALL PIXRHIST (MEMR(ADDA), NB, MEMI(ADDH), NT, BASE,
     1      INCR)
         CALL DATPUTR (HIST, 'BASE', BASE, 1)
         CALL DATPUTR (HIST, 'INCREMENT', INCR, 1)
      ELSE
         CALL ERRREPOR (ERRBDARG, ROUTINE, 'Array type '//TA//
     1      'Not supported')
      END IF
C
