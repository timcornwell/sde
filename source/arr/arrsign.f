...
C++
C @(#)arrsign.f	1.3    11/7/90
C
       SUBROUTINE ARRSIGN (A1, A2, A3)
C
CD Transfer the sign of A2 to A1
C
C
...
C
      IF (T1.EQ.'R') THEN
         CALL PIXRSIGN (MEMR(ADD1), MEMR(ADD2), MEMR(ADD3), NT)
      ELSE IF (T1.EQ.'I') THEN
         CALL PIXISIGN (MEMI(ADD1), MEMI(ADD2), MEMI(ADD3), NT)
      ELSE IF (T1.EQ.'D') THEN
         CALL PIXDSIGN (MEMD(ADD1), MEMD(ADD2), MEMD(ADD3), NT)
      ELSE
