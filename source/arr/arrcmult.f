...
C++
C @(#)arrcmult.f	1.3    11/7/90
C
      SUBROUTINE ARRCMULT (A1, A2, A3)
C
CD Multiply two arrays together, using conjugate of A2 if it is COMPLEX
C
C
C	A1	CHAR	input	Name of array
C	A2	CHAR	input	Name of array
C	A3	CHAR	input	Name of array
C Audit trail:
C	Original version: Audit trail comments go on this line
C	and successive lines
C				T.J.Cornwell	Jan 5 1989
C	Changed logic of axis checking
C				T.J.Cornwell	July 2 1990
C
C--------------------------------------------------------------------
#include	"stdinc.h"
C
...
C
      ...
      ELSE IF (T1.EQ.'X') THEN
         CALL PIXCXMUL (MEMX(ADD1), MEMX(ADD2), MEMX(ADD3), NT)
