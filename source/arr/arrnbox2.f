...
C++
C @(#)arrnbox2.f	1.1	 3/14/91
C
      SUBROUTINE ARRNBOX2 (NAME, WT, XINC, BEGIN, ENDING, NINT)
C
...
C       BEGIN   REAL    input   First value
C       ENDING  REAL    input   Last value 
...
C
      CHARACTER*(*)	NAME, WT
      REAL		XINC, BEGIN, ENDING
      INTEGER		NINT
C
      CHARACTER*(*)	ROUTINE

      PARAMETER		(ROUTINE = 'ARRNBOX2')
C
...
C
      IF (ATYPE.EQ.'R') THEN
         CALL PIXNBOX2(MEMR(ADD), MEMR(WADD), NAXIS(1),
     1                 XINC, BEGIN, ENDING, NINT)
      ELSE
