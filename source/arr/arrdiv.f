...
C++
C @(#)arrdiv.f	1.3    11/7/90
C
       SUBROUTINE ARRDIV (A1, A2, A3)
C
CD Divide one array by another: A3 = A1/A2
C
C
...
C
      IF (T1.EQ.'R') THEN
         CALL PIXRDIV (MEMR(ADD1), MEMR(ADD2), MEMR(ADD3), NT)
      ELSE IF (T1.EQ.'X') THEN
         CALL PIXXDIV (MEMX(ADD1), MEMX(ADD2), MEMX(ADD3), NT)
      ELSE IF (T1.EQ.'I') THEN
         CALL PIXIDIV (MEMI(ADD1), MEMI(ADD2), MEMI(ADD3), NT)
      ELSE IF (T1.EQ.'D') THEN
         CALL PIXDDIV (MEMD(ADD1), MEMD(ADD2), MEMD(ADD3), NT)
      ELSE
