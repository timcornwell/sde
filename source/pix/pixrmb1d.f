...
C++
C @(#)pixrmb1d.f	1.2    2/8/95
C
      SUBROUTINE PIXRMB1D (BEAM, INWIN, OUTWIN, PSF, WDELT,
     $   NIWIN, NOWIN, NPX, NIX, NOX, IBBLO, IBBHI,
     $   OBBLO, OBBHI)
C
CD Make a 2-d windowed beam matrix from a 1-d real PSF
C
...
C
     ...
     $   		WDELT
      REAL		BEAM(NIWIN,NOWIN), PSF(NPX)
      REAL		INWIN(NIX), OUTWIN(NOX)
C
      CHARACTER*(*)	ROUTINE
      PARAMETER		(ROUTINE = 'PIXRMB1D')
C
