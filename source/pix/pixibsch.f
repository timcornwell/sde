...
C++
C @(#)pixibsch.f	1.3    11/7/90
C
      SUBROUTINE PIXIBSCH (ARRAY, SIZE, VALUE, POSITION)
CD     
...
C    array      INTEGER input	data array to be searched
C    value      INTEGER input  value to search for
C
C    Audit trail:
C    Cloned from pixrbsch
C                                    R.G. Marson     Feb 15 1990
C    
C---------------------------------------------------------------------
#include	"stdinc.h"
C
      INTEGER           SIZE, POSITION, VALUE, ARRAY(SIZE)
C     
      CHARACTER*(*)	ROUTINE
      PARAMETER		(ROUTINE = 'PIXIBSCH')
C     
