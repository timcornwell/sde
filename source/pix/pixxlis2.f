...
C++
C @(#)pixxlis2.f	1.1    10/29/91
C
      SUBROUTINE PIXXLIS2 (ARRAY, NAX, NAXIS, WHAT, IMIN, IMAX,
     $   ISKIP, DORANGE, DOEXCLD, RMIN, RMAX)
C
CD List out arrays on MSGPUT  (Many argument version)
C
C	ARRAY	CMPLX(*) input	array.  (1D, indexed by hand)
...
C
            K = STRLEN(MESSAGE) + 2
            WRITE (MESSAGE(K:), 310) REAL(CVAL), AIMAG(CVAL)
 310        FORMAT ('(',1PG15.7E2,',',1PG15.7E2,')')
