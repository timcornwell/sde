...
C++
C @(#)pixddscat.f	1.3    11/7/90
C
      SUBROUTINE PIXDDSCAT (A, XSIZE, YSIZE, X, Y, DATA, NUMPTS)
CD     
C    Recover x, y, data values from a 2D array
C    
C    x	         REAL    output	x values extracted
C    y	         REAL    output	y values extracted
C    data        DOUBLE PRECISION output the data values extracted
C    a           DOUBLE PRECISION input	Input array (2 dimensional)
...
C     
      ...
      DOUBLE PRECISION  DATA(NUMPTS), A(XSIZE, YSIZE)
      REAL		X(NUMPTS), Y(NUMPTS)
C     
      CHARACTER*(*)	ROUTINE
      PARAMETER		(ROUTINE = 'PIXDDSCAT')
C     
      INTEGER		I, J, INDX
C=====================================================================
C     
