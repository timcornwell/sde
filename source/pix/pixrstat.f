...
C++
C @(#)pixrstat.f	1.5    10/21/94
C
      SUBROUTINE PIXRSTAT (A, N1, N2, N3, N4, N5, N6, N7, 
     $   BLC, TRC, DMAX, DMIN, AVE, RMS, SUM, DISP, NLOC, BBBLC, BBTRC,
     $   MINLOC, MAXLOC)
C
CD Calculate array statistics of REAL array only
C
...
C	Added DISPERSION. Fixed bug for MIN, MAX
C                               M.A.Holdaway    Feb 2 1990
C	Added BBBLC & BBTRC.
C				D.S.Briggs	Feb 27 1992
C	Bugfix in bounding box code.
...
C
     ...
     $   		MINLOC(*), MAXLOC(*)
      REAL 		DMAX, DMIN, AVE, RMS, SUM, DISP
      REAL		A(N1,N2,N3,N4,N5,N6,N7), T
C
      CHARACTER*(*)	ROUTINE
      PARAMETER		(ROUTINE = 'PIXRSTAT')
C
                  ...
                  DO 130 I3 = BLC(3), TRC(3)
                     DO 120 I2 = BLC(2), TRC(2)
                        DO 110 I1 = BLC(1), TRC(1)
                             DISP = DISP + (A(I1,I2,I3,I4,I5,I6,I7) -
     $                             AVE)**2
 110                    CONTINUE
 120                 CONTINUE
 130              CONTINUE
