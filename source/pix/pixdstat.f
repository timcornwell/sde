...
C++
C @(#)pixdstat.f	1.4  10/21/94
C
      SUBROUTINE PIXDSTAT (A, N1, N2, N3, N4, N5, N6, N7, 
     $   BLC, TRC, DMAX, DMIN, AVE, RMS, SUM, DISP, NLOC, BBBLC, BBTRC,
     $   MINLOC, MAXLOC)
C
CD Calculate array statistics of DOUBLE array only
C
C	A	DOUBLE	input	Input array
...
C
     ...
     $   		MAXLOC(*), MINLOC(*)
      ...
      DOUBLE PRECISION	A(N1,N2,N3,N4,N5,N6,N7), T
C
      CHARACTER*(*)	ROUTINE
      PARAMETER		(ROUTINE = 'PIXDSTAT')
C
...
C
      ...
      DISP = 0.0
      NLOC = 0
      DMAX = A(BLC(1),BLC(2),BLC(3),BLC(4),BLC(5),BLC(6),BLC(7))
      DMIN = A(BLC(1),BLC(2),BLC(3),BLC(4),BLC(5),BLC(6),BLC(7))
      NAX = 1
      IF (N2.GT.1) NAX = 2
      IF (N3.GT.1) NAX = 3
      IF (N4.GT.1) NAX = 4
...
C
      IF (NAX.EQ.1) THEN
         DO 210 I1 = BLC(1), TRC(1)
            IF (A(I1,1,1,1,1,1,1).NE.0.D0) THEN
               NLOC = NLOC + 1
      ...
      ELSE IF (NAX.EQ.2) THEN
         DO 320 I2 = BLC(2), TRC(2)
            DO 310 I1 = BLC(1), TRC(1)
               IF (A(I1,I2,1,1,1,1,1).NE.0.D0) THEN
                  NLOC = NLOC + 1
                  BBBLC(1) = MIN(BBBLC(1),I1)
         ...
         DO 430 I3 = BLC(3), TRC(3)
            DO 420 I2 = BLC(2), TRC(2)
               DO 410 I1 = BLC(1), TRC(1)
                  IF (A(I1,I2,I3,1,1,1,1).NE.0.D0) THEN
                     NLOC = NLOC + 1
                     BBBLC(1) = MIN(BBBLC(1),I1)
                     BBBLC(2) = MIN(BBBLC(2),I2)
                     ...
                     DO 530 I3 = BLC(3), TRC(3)
                        DO 520 I2 = BLC(2), TRC(2)
                           DO 510 I1 = BLC(1), TRC(1)
                              IF (A(I1,I2,I3,I4,I5,I6,I7).NE.0.D0) THEN
                                 NLOC = NLOC + 1
                                 BBBLC(1) = MIN(BBBLC(1),I1)
                                 BBBLC(2) = MIN(BBBLC(2),I2)
                                 BBBLC(3) = MIN(BBBLC(3),I3)
