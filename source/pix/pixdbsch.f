...
C++
C @(#)pixdbsch.f	1.3    11/7/90
C
      SUBROUTINE PIXDBSCH (ARRAY, SIZE, VALUE, POSITION)
CD     
...
C    array      DOUBLE  input	data array to be searched
C    value      DOUBLE  input  value to search for
C
C    Audit trail:
...
C
      ...
      DOUBLE PRECISION	ARRAY(SIZE), VALUE
C     
      CHARACTER*(*)	ROUTINE
      PARAMETER		(ROUTINE = 'PIXDBSCH')
C     
