...
C++
C @(#)pixxrdcl.f	1.1	 3/15/91
C
      SUBROUTINE PIXXRDCL (IMAGE, NX, NY, RX, RY, DX, DY,
     $   MINRAD, MAXRAD, CLIP)
C
CD CLIPS the IMAGE to CLIP outside of MAXRAD, inside of MINRAD
C
C	IMAGE	X ARR	in	2-D IMAGE to be clipped
C	NX	INT	in	X size of the IMAGE
C	RX	REAL	in	reference pixel
C	DX	REAL	in	pixel size, X (deg)
C	DY	REAL	in	pixel size, Y (deg)
C	MAXRAD	REAL	in	radius outside which we clip, DEG
C	MINRAD	REAL	in	radius outside which we clip, DEG
...
C
      INTEGER		NX, NY
      COMPLEX		IMAGE(NX, *)
      REAL		CLIP, RX, RY, DX, DY
      REAL		MAXRAD, MINRAD
C
      CHARACTER*(*)	ROUTINE
      PARAMETER		(ROUTINE = 'PIXXRDCL')
C
      COMPLEX		CCLIP
