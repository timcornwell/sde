...
C++
C @(#)pixdsubs.f	1.1    6/7/93
C
      SUBROUTINE PIXDSUBS (A, B, AN1, AN2, AN3, AN4, AN5, AN6,
     1   AN7, BN1, BN2, BN3, BN4, BN5, BN6, BN7, BLC, TRC, 
     1   STEP, SUM)
C
CD Subsection and Sum array
C
C	A	DBLE	input	Input array
C	B	DBLE	output	Output array
...
C       SUM     INT     input   How many pixels to sum over
C Audit trail:
...
C
      ...
      DOUBLE PRECISION	A(AN1, AN2, AN3, AN4, AN5, AN6, AN7)
      DOUBLE PRECISION	B(BN1, BN2, BN3, BN4, BN5, BN6, BN7)
C
      CHARACTER*(*)	ROUTINE
      PARAMETER		(ROUTINE = 'PIXDSUBS')
C
...
C
 ...
 666  CONTINUE
C
C     Loop once for each time we need to sum
C
      DO 170 K7 = 1, SUM(7)
       TBLC(7) = BLC(7) + K7 - 1
       DO 160 K6 = 1, SUM(6)
